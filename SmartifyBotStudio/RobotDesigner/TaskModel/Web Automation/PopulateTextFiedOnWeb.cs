...
{
    [Serializable]
    public class PopulateTextFiedOnWeb : RobotActionBase, ITask
    {
        ...
        public ObservableCollection<HtmlElementInfo> PopulateTextField_HtmlElemtnInfo_list { get; set; }
        ...
        public int Execute()
        {
            try
            {
                ...
                for (int i = 0; i < PopulateTextField_HtmlElemtnInfo_list.Count; i++)
                {

                    if (PopulateTextField_HtmlElemtnInfo_list[i].Html_element_id != "")
                    {
                        element = working_driver.FindElement(By.Id(PopulateTextField_HtmlElemtnInfo_list[i].Html_element_id));
                    }
                    else if (PopulateTextField_HtmlElemtnInfo_list[i].Html_element_XPATH != "")
                    {
                        element = working_driver.FindElement(By.XPath(PopulateTextField_HtmlElemtnInfo_list[i].Html_element_XPATH));
                    }
                    else
                    {
                        element = working_driver.FindElement(By.Name(PopulateTextField_HtmlElemtnInfo_list[i].Html_element_name));
                    }

                    element.SendKeys(PopulateTextField_HtmlElemtnInfo_list[i].Html_element_input_text);
                }
